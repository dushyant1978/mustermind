...
/** Only codes we ship fixtures for, unless explicitly opened up by env. */
export function isAllowedStyleCode(code) {
  ...
  return DEMO_STYLE_CODES.includes(code);
}
...
/**
 * Ordinary app headers only. Deliberately does not include any sensor or
 * anti-bot value. `ad_id` is a random per-process UUID.
 */
function pdpHeaders() {
  return {
...
export async function getDeliveryEvidence(sku, pin, { styleCode, productProvenance = 'live' } = {}) {
  ...
  const live = await fetchJson(url, { Accept: 'application/json' });
  let result;
...
export async function searchCandidates(query, pincode = '560029', pageSize = 8) {
  ...
  if (!q || q.length > SEARCH_QUERY_MAX_LEN || !SEARCH_QUERY_ALLOWED.test(q)) {
    return { ok: false, error: 'invalid search query', codes: [] };
  }
  ...
  const r = await fetchJson(`${SEARCH_BASE}?${params.toString()}`, { Accept: 'application/json' });
